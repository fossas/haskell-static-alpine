...
# - bootstrapping cabal-install 3.x: cabal zlib-dev
...
#
# We "install" cabal-install twice here because cabal-install 2.4.x doesn't
# support --installdir or --install-method=copy. The second install is fast.
...
# HACK: the 8.8 musl bindist from ghcup expects "libtinfow.so.6". libtinfo
# is integrated into libncursesw.so.6 -- so copy the symlink
#
# We can remove this if/when the musl bindists link directly against ncurses again.
RUN cp /usr/lib/libncursesw.so.6 /usr/lib/libtinfow.so.6
RUN cp /usr/lib/libncursesw.so.6 /usr/lib/libtinfow.so

# Uninstall GHC 8.6.5 && install 8.8.2
RUN ghcup rm --force 8.6.5 && ghcup install 8.8.2 && ghcup set 8.8.2
